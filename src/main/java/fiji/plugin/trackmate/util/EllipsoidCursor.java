...
public class EllipsoidCursor< T > extends AbstractNeighborhoodCursor< T >
{
	...
	public EllipsoidCursor( final AbstractNeighborhood< T > ellipsoid )
	{
		super( ellipsoid );
		/*
	...
	public void fwd()
	{
		...
		case INITIALIZED:
			...
			TMUtils.getXYEllipseBounds( ( int ) neighborhood.span[ largeAxisDim ],
					( int ) neighborhood.span[ 2 ], rys );
			ry = rys[ 0 ];

			TMUtils.getXYEllipseBounds( ( int ) neighborhood.span[ smallAxisdim ],
		...
		case INCREMENT_Y:
			...
			position[ largeAxisDim ] = -position[ largeAxisDim ] + 1;
			// y should be negative (coming from mirroring or init = 0)
		...
		case INCREMENT_Z:
			...
			TMUtils.getXYEllipseBounds(
					Math.round( ( float ) ry * neighborhood.span[ smallAxisdim ]
							/ neighborhood.span[ largeAxisDim ] ),
					ry, rxs );
			rx = rxs[ 0 ];
	...
	/**
	 * @return a reference to the current relative position array, measured with
	 *         respect to the neighborhood center.
	 */
	 ...
	 *         <p>
	 *         In spherical coordinates, the inclination is the angle between
	 *         the Z axis and the line OM where O is the sphere center and M is
	 *         the point location.
	 */
	 ...
	 *         <p>
	 *         In spherical coordinates, the azimuth is the angle measured in
	 *         the plane XY between the X axis and the line OH where O is the
	 *         sphere center and H is the orthogonal projection of the point M
	 *         on the XY plane.
	 */
